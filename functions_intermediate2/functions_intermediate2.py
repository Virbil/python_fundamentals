...
x = [ [5,2,3], [10,8,9] ]  # list of lists is the hardest to understand/traverse
...
sports_directory = {    #dictionary with lists as values(?)
...
search = 10
for num_replace in x:
    if num_replace[0] == search:
        num_replace[0] = 15
        break
# print(x)
...
# loops through each dictionary in the list and prints each key and the associated value. For 
...
def iterate_dictionary(provided_list_of_dicts):
    for names in provided_list_of_dicts:
        print(names)
...
def iterate_dictionary2(key_name, list_of_dicts):
    for names in list_of_dicts:
        print(names[key_name])

iterate_dictionary2('last_name', new_students)
...
def print_info(provided_dict_of_lists):
    for items, details in provided_dict_of_lists.items():
        print(f"{len(details)} {items.upper()}: ")
